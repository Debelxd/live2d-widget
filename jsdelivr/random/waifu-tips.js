...
!(function () {
  ...
  function switchLanguage() {
    const currentLang = localStorage.getItem('waifu-language') || 'es';
    ...
    localStorage.setItem('waifu-language', nextLang);
    ...
    if (typeof showMessage !== 'undefined') {
      showMessage(messages[nextLang], 2000, 9);
    } else {
      o(messages[nextLang], 2000, 9);
    }
    ...
    async loadModel(t, s, i) {
      if (
        ...
        loadlive2d("live2d", `${this.apiPath}get/?id=${t}-${s}`),
          console.log(`Live2D Model ${t}-${s} Loading completed`);
    }
    async loadRandModel() {
      ...
      if (this.useCDN) {
        ...
        loadlive2d("live2d", `${this.cdnPath}model/${s}/index.json`),
          o("Do my new clothes look good?", 4e3, 10);
      } else
        fetch(`${this.apiPath}rand_textures/?id=${t}-${s}`)
          .then((e) => e.json())
          .then((e) => {
            1 !== e.textures.id || (1 !== s && 0 !== s)
              ? this.loadModel(t, e.textures.id, "Do my new clothes look good?")
              : o("I don’t have any other clothes yet!", 4e3, 10);
          });
    }
  }
    ...
    hitokoto: {
      ...
      callback: function () {
        fetch("https://v1.hitokoto.cn")
          .then((e) => e.json())
          .then((e) => {
            const t = `This sentence comes from <span>「${e.from}」</span>，yes <span>${e.creator}</span> exist hitokoto.cn Post-like.`;
            o(e.hitokoto, 6e3, 9),
              setTimeout(() => {
                o(t, 4e3, 9);
              }, 6e3);
    ...
    asteroids: {
      ...
      callback: () => {
        if (window.Asteroids)
          ...
          (e.src =
            "https://fastly.jsdelivr.net/gh/nova1751/live2d-api@latest/asteroids.js"),
    ...
    quit: {
      ...
      callback: () => {
        localStorage.setItem("waifu-display", Date.now()),
          o("May you reunite with your special someone one day.", 2e3, 11),
    ...
    function c(t) {
      ...
      window.addEventListener("mousemove", () => (n = !0)),
          ...
          (function (e) {
            ...
            if ("" !== document.referrer) {
              const e = new URL(document.referrer),
                ...
                i = { baidu: "Baidu", so: "360search", google: "Google search" };
              return location.hostname === e.hostname
                ? t
                : ((o = s in i ? i[s] : e.hostname),
                  `Hello！from <span>${o}</span> friends<br>${t}`);
            }
            return t;
          })(t.time),
          7e3,
        ...
        window.addEventListener("mouseover", (s) => {
          for (let { selector: n, text: c } of t.mouseover)
            if (s.target.closest(n)) {
              if (i === n) return;
              return (
                (i = n),
                (c = e(c)),
                (c = c.replace("{text}", s.target.innerText)),
                void o(c, 4e3, 8)
              );
            }
        }),
